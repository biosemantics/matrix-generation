...
public class NexusWriter implements Writer {
	...
	private void writeMatrix(BufferedWriter bw, RawMatrix rawMatrix, Map<Integer, List<String>> columnStates) throws IOException {
		bw.append("matrix\n");
		...
		for(RowHead rowHead : rawMatrix.getRowHeads()) {
			...
			for(int i=0; i<rowValues.size(); i++) {
				String state = rowValues.get(i).getValue();
	...
	private void writeMetaData(BufferedWriter bw, RawMatrix rawMatrix, Map<Integer, List<String>> columnStates) throws IOException {
		bw.append("#nexus\n");
	...
	private int getMaxRowHeadLength(RawMatrix rawMatrix) {
		int result = 0;
		for(RowHead rowHead : rawMatrix.getRowHeads()) {
	...
	private Map<Integer, List<String>> getColumnStates(RawMatrix rawMatrix) {
		Map<Integer, List<String>> columnStates = new HashMap<Integer, List<String>>();
		for(int i=0; i<rawMatrix.getColumnCount(); i++) {
			Set<String> states = new HashSet<String>();
			for(RowHead rowHead : rawMatrix.getRowHeads()) {
				states.add(rawMatrix.getCellValues().get(rowHead).get(i).getValue());
			}
