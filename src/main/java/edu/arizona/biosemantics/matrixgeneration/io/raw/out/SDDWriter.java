...
public class SDDWriter implements Writer {
	...
	private class DatasetHandler {
		...
		public void create(edu.arizona.biosemantics.matrixgeneration.model.raw.Matrix matrix) {
			...
			String label = "";
			for(RowHead rowHead : matrix.getRootRowHeads()) {
			...
			for(RowHead rowHead : matrix.getRootRowHeads()) {
				...
				characterSetHandler.processRowHead(rowHead, matrix);
				characterTreeHandler.processRowHead(rowHead);
				descriptiveConceptHandler.processRowHead(rowHead, matrix);
			}
	...
	private class DescripitiveConceptHandler {
		...
		public void processRowHead(RowHead rowHead, Matrix matrix) {
			...
			for(ColumnHead columnHead : columnHeads) {
				if(columnHead.hasCharacterSource()) {
					...
					if(!createdStructures.contains(structureIdentifier)) {
						...
						modifierHandler.processColumnHead(columnHead, matrix);
						characterTreeHandler.processTaxonConceptStructureTriple(
								new TaxonConceptStructureTriple(rowHead, dc, structureIdentifier), matrix);
					}
				}
			}
		}
	...
	private class ModifierHandler {
		...
		public void processColumnHead(ColumnHead columnHead, Matrix matrix) {
			//loop over each character name in the structure's char->state map
			int index = matrix.getColumnHeads().indexOf(columnHead);
			for(List<CellValue> values : matrix.getCellValues().values()) {
				CellValue cellValue = values.get(index);
				String modifier = cellValue.getSource().getRepresentative().getModifier();
				if(modifier != null) {
		...
		public void processPossibleModifier(Map<TaxonCharacterStateTriple, CellValue> possibleModifier) {
			...
			if(cellValue.getSource().getRepresentative().getModifier() != null) {
				TaxonCharacterStateTriple triple = possibleModifier.keySet().iterator().next();
				ModifierDef modDef = seenModifiers.get(cellValue.getSource().getRepresentative().getModifier());
	...
	private class CharacterSetHandler {
		...
		public void processRowHead(RowHead rowHead, Matrix rawMatrix) {
			...
			for(int i=0; i<rawMatrix.getColumnCount(); i++) {
				...
				if(columnHead.hasCharacterSource()) {
					...
					Value value = cellValue.getSource().getRepresentative();
					...
					case SINGLETON_CATEGORICAL:
						...
						localStateDef = makeSingleCategoricalState(cellValue.getText());
		...
		private void attachStateCategorical(CategoricalCharacter character,
			...
			for(Map<AbstractCharacterDefinition, Set<CharacterLocalStateDef>> taxonMap 
				...
				for(AbstractCharacterDefinition presentChar : taxonMap.keySet()) {
					...
					if(!flaggedGlobal && stateSet.contains(localStateDef) &&
						...
						AbstractRef stateRef = stateRefs.get(cellValue.getText());
			...
			List<Object> currentStates = 
					character.getStates().getStateDefinitionOrStateReference();
			if(!flaggedGlobal && !stateRefs.containsKey(cellValue.getText())) {
				...
				stateRefs.put(cellValue.getText(), stateRef);
			}
			else if(!flaggedGlobal){
				if(!currentStates.contains(stateRefs.get(cellValue.getText())))
					currentStates.add(stateRefs.get(cellValue.getText()));
			}
		}
		...
		private void putMappingAndRangeOnQuantitativeCharacter(
			...
			if (valueTypeDeterminer.isSingleton(cellValue.getSource().getRepresentative())) {
			...
			else if (valueTypeDeterminer.isRange(cellValue.getSource().getRepresentative())) {
				QuantitativeCharMapping mapping = objectFactory.createQuantitativeCharMapping();
				ValueRangeWithClass range = objectFactory.createValueRangeWithClass();
				range.setLower(Double.valueOf(from));
				range.setUpper(Double.valueOf(to));
	...
	private class CharacterTreeHandler {
		...
		public void processTaxonConceptStructureTriple(TaxonConceptStructureTriple triple, Matrix matrix) {
		...
		private void addConceptNodesToCharacterTree(RowHead rowHead, DescriptiveConcept dc, StructureIdentifier structureIdentifier, 
			...
			if(completeMatrix.getParent(structureIdentifier) != null) {
				StructureIdentifier parentStructure = completeMatrix.getParent(structureIdentifier);
				CharTreeNodeRef ctNodeRef = objectFactory.createCharTreeNodeRef();
	...
	@Override
	public void write(Matrix matrix) throws DatatypeConfigurationException, IOException, JAXBException {
		Datasets datasets = createDatasets(matrix);
		marshaller.marshal(datasets, file);			
	}

	private Datasets createDatasets(Matrix matrix) throws DatatypeConfigurationException, IOException {	
		Datasets datasets = objectFactory.createDatasets();
		metdataCreator.create(datasets);
		datasetHandler.create(matrix);
